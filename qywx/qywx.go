...
// 企业微信的信息，新增程序只要把数据放在这里就行
func qywxValues() []gin.H {
	return []gin.H{
		{
			"host":       "127.0.0.1",
			"agentid":    "1000012",
			"corpsecret": "HKQiLNj9WniKJcyoO3Qs6B9kP6M7zBwziP31EQ1SSwg",
		},
	}
}

var corpId = "wwedd17ec8654d77fe"
var defaultQywxValue = qywxValues()[0]
var access_token = QywxAccessToken(defaultQywxValue["corpsecret"].(string))
...
func QywxAccessToken(corpsecret string) string {
	if len(corpsecret) == 0 {
		corpsecret = defaultQywxValue["corpsecret"].(string)
	}
	valueMap := QywxHttpGet(fmt.Sprintf("https://qyapi.weixin.qq.com/cgi-bin/gettoken?corpid=%s&corpsecret=%s", corpId, corpsecret))
...
func qywxRequestGet(url string, urlEnd string) map[string]interface{} {
	if len(access_token) == 0 {
		access_token = QywxAccessToken(defaultQywxValue["corpsecret"].(string))
	}
